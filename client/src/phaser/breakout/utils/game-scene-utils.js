...
export function addPlayer(scene, player) {
  scene.gameState.player = scene.physics.add.sprite(
    scene.cameras.main.centerX,
    scene.cameras.main.displayHeight - 50,
    player
  );
...
export function addBall(scene, ball) {
  scene.gameState.ball = scene.physics.add
    .sprite(scene.cameras.main.centerX, scene.cameras.main.centerY, ball)
    .setScale(2);
...
export function addPlatform(scene, platform) {
  ...
  scene.gameState.platforms
    .create(
      scene.cameras.main.centerX,
      scene.cameras.main.displayHeight - 5,
      platform
    )
    .setScale(1, 0.01)
    .refreshBody();
...
export function addColliders(scene) {
  scene.physics.add.collider(
    scene.gameState.ball,
    scene.gameState.player,
    () => {
      const velocityX =
        scene.gameState.ball.body.velocity.x +
        scene.gameState.player.body.velocity.x *
          (scene.gameState.ballSpeed / MAX_VELOCITY);
...
export function genBrickGroup(brickGroup) {
  for (let yBricks = 1; yBricks < 8; yBricks++) {
    for (let xBricks = 1; xBricks < 16; xBricks++) {
      brickGroup
        .create(34 * xBricks, 50 + 17 * yBricks, `brick${yBricks}`)
        .setImmovable(true);
    }
  }
}
...
export function initTextDisplay(scene) {
  scene.gameState.scoreDisplay = scene.add.text(
    20,
    10,
    `Score: ${scene.gameState.score}`,
    TEXT_STYLE
  );
  scene.gameState.highScoreDisplay = scene.add
    .text(
      scene.cameras.main.displayWidth - 20,
      10,
      `High Score: ${scene.gameState.highScore}`,
      TEXT_STYLE
    )
    .setOrigin(1, 0);
  scene.gameState.levelDisplay = scene.add.text(
    scene.cameras.main.centerX,
    10,
    `${scene.gameState.level}`,
    TEXT_STYLE
  );
}
...
export function updateScoreDisplay(scene) {
  ...
  scene.gameState.highScoreDisplay.setText(
    `High Score: ${scene.gameState.highScore}`
  );
}
...
function displayGameOver(scene) {
  scene.add
    .text(
      ...
      'Game Over!\n\nPlay Again?',
      { ...TEXT_STYLE, fontSize: '24px', fill: '#ff0000' }
    )
    .setOrigin(0.5);

  scene.gameState.yes = scene.add
    .text(scene.cameras.main.centerX, scene.cameras.main.centerY + 100, 'YES', {
      ...TEXT_STYLE,
      fontSize: '24px',
      fill: '#ff0000',
    })
    .setOrigin(0.5)
    .setInteractive();
  scene.gameState.no = scene.add
    .text(scene.cameras.main.centerX, scene.cameras.main.centerY + 150, 'NO', {
      ...TEXT_STYLE,
      fontSize: '24px',
      fill: '#ff0000',
    })
  ...
  interactiveButtons(scene, [scene.gameState.yes, scene.gameState.no]);
}
...
function updateHighScore(scene) {
  scene.gameState.highScore =
    scene.gameState.highScore >= scene.gameState.score
      ? scene.gameState.highScore
